...
[
  ...
  },
  {
    ...
    "line": null,
    "description": "06-04-PLAN.md's AUDIO-03 implementation used D-12's FALLBACK as its primary mechanism and never implemented D-12's decided one. DemuxSession::open() called compute_sbr_signaling() unconditionally, and for EVERY AAC stream whose AudioSpecificConfig lacked EXPLICIT SBR -- i.e. every ordinary AAC-LC file -- probe_implicit_sbr_via_second_open() performed a complete SECOND avformat_open_input + avformat_find_stream_info of the same file. Caught by the PERF-03 instruction ratchet on CI run 35713912901: plain_instructions 257709408 -> 311206951 (+20%, tolerance +/-2%), full_instructions 344956981 -> 399298173 (+15%). Localised by callgrind (debug session .planning/debug/resolved/phase6-packet-scan-instr-20pct.md): probe_implicit_sbr_via_second_open inclusive = 53,343,680 Ir of 310,678,351 (17.2%), of which 50,933,555 (95.5%) was the second container open and only ~60,000 Ir (0.11%) the bounded decode the probe exists to perform -- its two documented DoS bounds (kMaxSbrProbePackets=1, kMaxSbrProbeContainerPacketsScanned=64) bound only that 0.11%, nothing bounded the open. Differential builds (pre-phase b00eec7 vs HEAD vs HEAD-minus-the-probe, all under valgrind in an ubuntu:24.04 container) attributed 98.4% of the regression to it; everything else in Phase 6 cost +0.33% on the plain leg. SECOND, INDEPENDENT DEFECT from the same code: on ordinary encoder-produced AAC the probe returned no answer at all. The first packet is consumed entirely as 1024-sample encoder-delay priming (AV_PKT_DATA_SKIP_SAMPLES start_skip=1024), so avcodec_receive_frame returns EAGAIN after the single allowed packet and resolve_sbr_signaling fell through to SbrSignaling::unknown -- audio.profile read 'LC (sbr: unknown)' for every normally-encoded AAC file in the corpus, while only the priming-free hand-written fixtures (audio_sbr_implicit.mp4, audio_aac_handwritten.mp4) ever got a frame, which is why 06-04's own verification passed. D-14 makes unknown compare as its own value, so that was a latent false finding on unchanged media. FIXED by 06-13: D-12's primary mechanism implemented as decided -- the post-find_stream_info codecpar (profile, plus the ASC core rate against the resolved rate) answers first, with the bounded probe retained as the fallback D-12 always called it, reachable only when find_stream_info resolved no profile at all. Verified against the linked FFmpeg 8.1 (never the system ffmpeg) over every AAC fixture in this corpus: all resolve a real profile, and audio_sbr_implicit.mp4 resolves BOTH the AV_PROFILE_AAC_HE profile and the doubled 88200 Hz rate. Post-fix: timeline plain 258,959,965 (+0.48% vs the committed baseline) and full 347,052,373 (+0.61%), both inside tolerance with PERF_BASELINE.txt's timeline lines untouched; ordinary AAC now reports 'LC'. CONFIRMED on the real designated leg by 06-13-PLAN.md's CI round trip (run 35735099865, commit e5a16770c00343da91ad8e299773eac9774a1bd3): the Timeline instruction-count ratchet gate succeeded, matching this fix's own local measurement rather than repeating the prior +20%/+15% failure.",
    "status": "fixed",
    "reason": "",
    "recorded_at": "2026-09-22T13:02:31.549Z",
    "resolved_at": "2026-09-22T14:07:34.325Z"
  },
  {
    "id": 39,
    "kind": "deviation",
    ...
    "file": "src/probe/audio_decode.cpp",
    "line": null,
    "description": "06-13-PLAN.md Task 2 (D-06 cross-architecture proof): the real arm64-osx CI round trip (run 35735099865, commit e5a1677) proved only aac_fixed's class-1 cross-architecture bit-exactness (D-11's committed two-build proof, tests/integration/test_audio_hash_decoder.cpp class proof Test 2, run against the hand-written byte-identical audio_aac_handwritten.mp4 fixture). D-06's own mp3/mp2 promotion required bit-exact proof across architectures, not one x86 host; no trustworthy proof exists for either -- mp2's only real fixture (audio_mp2_base.mpg) is ffmpeg-encoder output already documented (WINDOWS.md #12) as not architecture-stable even under -flags +bitexact, and mp3 has no real corpus fixture at all in this LGPL decode-only pin. Per this plan's own must-have (confirmed-or-demoted, never closed on assumption), both are DEMOTED: determinism_class_for_decoder() now returns class 2 for mp3/mp2 (still selected by name under auto, per D-06's own T-06-15 selection/classification independence, now recorded class2 with a path_signature). ac3_fixed's cross-architecture bit-exactness is EQUALLY unmeasured by any real fixture-level test (no real AC-3 corpus fixture exists either) but its class-1 status predates D-06's reopening and is not the literal subject of this plan's must-have, so it is left unchanged and recorded here as an open gap rather than silently assumed. Building a trustworthy proof for mp3/mp2/ac3_fixed needs a D-10-style hand-written, architecture-stable elementary stream, which does not exist and is out of this plan's scope.",
    "status": "open",
    "reason": "",
    "recorded_at": "2026-09-22T14:04:13.122Z",
    "resolved_at": null
  },
  {
    "id": 40,
    ...
    "file": "src/probe/audio_decode.cpp",
    "line": null,
    "description": "CR-01 residual: the libav-internal path where a stream's first packets fail inside avformat_find_stream_info but decode later is not reproduced with real media (.planning/debug/audio-sweep-rate-truncation.md's own session could not construct it, and new ffmpeg-encoded media is out of scope). Its end state, codecpar keeping the header rate while the decoder emits another, is covered in-process by the CR-01 oracle test ('audio_decode - the sweep is configured from the decoded frame's rate even when codecpar declares a different one (CR-01)', tests/unit/test_audio_decode.cpp). Since 06-15 the sweep configures every sink from the decoded frame in any case, so this residual is diagnostic-only, not a live gap.",
    "status": "open",
    "reason": "",
    "recorded_at": "2026-09-23T20:10:26.407Z",
    "resolved_at": null
  },
  {
    "id": 41,
    ...
    "file": "src/probe/audio_decode.cpp",
    "line": null,
    "description": "T-06-55 (Denial of Service, low, accept): dropout_window_ (a std::deque<std::int64_t>) holds up to dropout_window_samples_ = max(1, sample_rate * kDropoutRmsWindowMs(100) / 1000) entries -- i.e. up to sample_rate/10 int64 (8-byte) values, so a crafted declared sample rate directly sizes this one deque. Growth is bounded by the samples ACTUALLY DECODED, not by the declared rate alone: for PCM (the format most exposed, since its rate is taken directly from the container header with no codec-side ceiling), the deque can reach at most about 4x the raw PCM input size (one int64 per interleaved sample, versus 2 bytes/sample for s16 or up to 8 for s64 -- worst case roughly 4x for common 16-bit PCM), because the number of decoded samples is itself bounded by how many sample-frames worth of bytes the packet stream actually contains. Compressed codecs additionally cap the rate in their own bitstream headers (FLAC's STREAMINFO sample-rate field is a 20-bit value, topping out near 1,048,575 Hz -- nowhere near the pathological INT_MAX rates size.* checks already guard against for other fields). A crafted rate therefore amplifies memory in PROPORTION TO the input size actually supplied, never unboundedly independent of it -- distinct from an unbounded-allocation vulnerability, where memory grows independent of input size.",
    "status": "waived",
