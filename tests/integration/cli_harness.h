...
inline CliResult spawn_and_capture(const std::string& executable, const std::vector<std::string>& args,
                                    const EnvVars* env_override) {
  CliResult result;
  ...
  if (!CreatePipe(&out_read, &out_write, &sa, 0) || !CreatePipe(&err_read, &err_write, &sa, 0)) {
    result.err = "cli_harness: CreatePipe failed";
    return result;
  }
  ...
  if (!ok) {
    ...
    result.err = "cli_harness: CreateProcess failed";
    return result;
  }
...
inline CliResult spawn_and_capture(const std::string& executable, const std::vector<std::string>& args,
  ...
  if (pipe(out_pipe) != 0 || pipe(err_pipe) != 0) {
    result.err = "cli_harness: pipe() failed";
    return result;
  }
  ...
  pid_t pid = -1;
  int spawn_rc = posix_spawn(&pid, executable.c_str(), &actions, nullptr, argv.data(), env_to_use);
  ...
  if (spawn_rc != 0) {
    close(out_pipe[0]);
    close(err_pipe[0]);
    result.err = "cli_harness: posix_spawn failed";
    return result;
  }
...
inline CliResult run_cli(const std::vector<std::string>& args, const EnvVars* env_override = nullptr) {
  return detail::spawn_and_capture(MEDIADIFF_BINARY, args, env_override);
}
