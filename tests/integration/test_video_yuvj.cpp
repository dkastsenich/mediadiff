...
// `video_yuv420p_pc.mp4`, `video_yuv420p_tv.mp4`) were themselves narrowed
// during this plan's own Task 3 (see 04-08-SUMMARY.md's Deviations
// section) from a `testsrc2` gradient source to a flat `color=c=gray`
// source specifically because the gradient source's own tv/pc JPEG
// re-quantization produced a real ~8% file-size delta that tripped
// `size.file`/`size.stream_bitrate`/`size.peak_bitrate` under
...
TEST_CASE("video_yuvj - yuvj420p vs yuv420p-full-range (the SAME intent, two spellings) produces ZERO non-pass "
          "findings across the whole report",
          "[integration]") {
  const nlohmann::ordered_json report =
      compare_json(fixture("video_yuvj420p.mp4"), fixture("video_yuv420p_pc.mp4"), "sw-encoder");
